...
1. Github码仓新增Jenkinsfile文件，内容格式参考：
	ü https://github.com/kraigyang/driver_display/blob/main/Jenkinsfile

2. Github端配置（Web页面：https://github.com/kern-crates/${repo}）：
	ü Repo Left Settings -> Webhooks -> Payload URL:http://49.51.192.19:9095/github-webhook/ -> Send me everything -> 保存
  ü Repo Right Settings -> Developer Settings -> Personal access tokens -> Tokens(classic) -> Generate -> 保存

3. Jenkins端配置（Web页面：http://49.51.192.19:9095）：
	ü Dashboard -> 系统管理 -> 凭据 -> 系统 -> 全局凭据 (unrestricted) -> Secret Text -> 填入上步中Generate的Token -> ID命名为"GithubAccessToken"（示例） -> 保存
	ü Pipeline -> Configuration -> 构建触发器（Github hook trigger for GITScm polling）-> 流水线（Pipeline script from SCM） -> Add CI monitored repo1/credentials1 -> Add CI monitored related repo2/credentials2 -> 保存
